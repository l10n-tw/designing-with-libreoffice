...
init(){
	...
	local overall_check_result='still-success' # or fail
	...
	IFS=$original_ifs; unset original_ifs; 
	popd >/dev/null

	if [ "${overall_check_result}" == 'fail' ]; then
		printf --\
			"%s: Syntax checking failed!\n"\
			"${RUNTIME_EXECUTABLE_NAME}"
		exit 1
	fi; unset overall_check_result
	exit 0
}; declare -fr init
